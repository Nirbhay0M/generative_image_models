...
tf.flags.DEFINE_string("data_dir", "./tmp/DRAW_EXPT1", "")
...
local_seed_left = np.random.randint(0,100000000,T)
local_seed_right = np.random.randint(0,100000000,T)

## BUILD MODEL ## 
ipl_steps = 10
e_fixed = [0]*T
e_left,e_right,e_ipl = [0]*T,[0]*T,[0]*T
e_temp = [0]*T
for i in range(T):
    e_temp[i] = [0]*(batch_size/ipl_steps)

# e_fixed = tf.random_normal((1,z_size), mean=0, stddev=1, seed=local_seed)
# e_fixed = tf.tile(e_fixed,[batch_size,1])
# # for i in range(e_test_size):

for i in range(T):
    e_fixed[i] = tf.random_normal((1,z_size), mean=0, stddev=1, seed=local_seed)
    e_fixed[i] = tf.tile(e_fixed[i],[batch_size,1])
    
    # print "local_seed_left:",local_seed_left[i]
    e_left[i] = tf.random_normal((batch_size/ipl_steps,z_size), mean=0, stddev=1, seed=local_seed_left[i])
    e_right[i] = tf.random_normal((batch_size/ipl_steps,z_size), mean=0, stddev=1, seed=local_seed_right[i])

    for j in range(ipl_steps):
        r = (1.0*j)/(ipl_steps-1)
        e_temp[i][j] = r*e_left[i] + (1-r)*e_right[i]

    e_ipl[i] = tf.concat(0,e_temp[i])
    
    e_left[i] = tf.tile(e_left[i],[batch_size,1])
    e_right[i] = tf.tile(e_right[i],[batch_size,1])
# assert False
...
# [e] Experiment
cs_evar=[0]*T
dec_state_evar=lstm_dec.zero_state(batch_size, tf.float32)

for t in range(T):
    c_prev_evar = tf.zeros((batch_size,img_size)) if t==0 else cs_evar[t-1]
    # z_evar=e_fixed[t] # CONTROL
    # z_evar=e_fixed[t] if t == 0  else e[t]
    # z_evar=e[t] if t == 0 else e_fixed[t]

    # z_evar=e[t] if t == T-1 else e_fixed[t]
    # z_evar=e_fixed[t] if t == T-1 else e[t]

    # z_evar = e_left[t]
    # z_evar = e_right[t]
    z_evar = e_ipl[t]

    h_dec_evar,dec_state_evar=decode(dec_state_evar,z_evar)
    cs_evar[t]=c_prev_evar+write(h_dec_evar)
...
# if e_test_size > 0:
#     print "Beginning [e] sample experiment."
